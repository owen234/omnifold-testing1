...
#include "corgen.c"
   ...
   void toy_chi2_test2b( double a0=0. , double b0=0.4, int nshow=25,  int ngen=10000   ) {
      ...
      for ( int i=0; i<ngen; i++ ) {
         ...
         x0 = a0 - b0 + rnvec(0) ;
         x1 = a0 - b0*0.33333 + rnvec(1) ;
         x2 = a0 + b0*0.33333 + rnvec(2) ;
         x3 = a0 + b0 + rnvec(3) ;
         ...
         for ( int ai=0; ai<nscan; ai++ ) {
            ...
            for ( int bi=0; bi<nscan; bi++ ) {
               ...
               chi2_basic += pow( (x1-(ascan-bscan*0.33333)), 2 ) / covmat(1,1) ;
               chi2_basic += pow( (x2-(ascan+bscan*0.33333)), 2 ) / covmat(2,2) ;
         ...
         for ( int ai=0; ai<nscan; ai++ ) {
            ...
            for ( int bi=0; bi<nscan; bi++ ) {
               ...
               deltaVec(1) = x1 - (ascan-bscan*0.33333) ;
               deltaVec(2) = x2 - (ascan+bscan*0.33333) ;
         ...
         if ( i < nshow ) {
            ...
            can -> cd(2) ;
            h_data -> Draw("pe") ;
            tl_basic -> DrawLine( -1., best_chi2_basic_aval - best_chi2_basic_bval,  1.,  best_chi2_basic_aval + best_chi2_basic_bval ) ;
            tl_full -> DrawLine( -1., best_chi2_full_aval - best_chi2_full_bval,  1.,  best_chi2_full_aval + best_chi2_full_bval ) ;
