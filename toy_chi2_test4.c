...
//
//  This one fits a parabola:  f = a + b * x^2
//

#include "corgen.c"


   void toy_chi2_test4( double a0=0. , double b0=1.4, int nshow=25,  int ngen=10000   ) {
      ...
      TH1F* h_data = new TH1F( "h_data", "data", 4, -1.3333, 1.3333 ) ;
      ...
      for ( int i=0; i<ngen; i++ ) {
         ...
         x0 = a0 + b0 + rnvec(0) ;
         x1 = a0 + b0*(0.33333*0.33333) + rnvec(1) ;
         x2 = a0 + b0*(0.33333*0.33333) + rnvec(2) ;
         x3 = a0 + b0 + rnvec(3) ;
         ...
         for ( int ai=0; ai<nscan; ai++ ) {
            ...
            for ( int bi=0; bi<nscan; bi++ ) {
               ...
               double chi2_basic(0.) ;
               chi2_basic += pow( (x0-(ascan+bscan)), 2 ) / covmat(0,0) ;
               chi2_basic += pow( (x1-(ascan+bscan*0.33333*0.33333333)), 2 ) / covmat(1,1) ;
               chi2_basic += pow( (x2-(ascan+bscan*0.33333*0.33333333)), 2 ) / covmat(2,2) ;
               chi2_basic += pow( (x3-(ascan+bscan)), 2 ) / covmat(3,3) ;
         ...
         for ( int ai=0; ai<nscan; ai++ ) {
            ...
            for ( int bi=0; bi<nscan; bi++ ) {
               double bscan = b_min + bi*(b_max-b_min)/(nscan-1) ;
               TVectorD deltaVec(4) ;
               deltaVec(0) = x0 - (ascan+bscan) ;
               deltaVec(1) = x1 - (ascan+bscan*0.33333*0.3333333) ;
               deltaVec(2) = x2 - (ascan+bscan*0.33333*0.3333333) ;
               deltaVec(3) = x3 - (ascan+bscan) ;
         ...
         if ( i < nshow ) {
            ...
            char func_str[1000] ;
            sprintf( func_str, "%8.4f+x*x*(%8.4f)", best_chi2_basic_aval, best_chi2_basic_bval ) ;
            TF1* tf_basic = new TF1( "tf_basic", func_str, -1., 1. ) ;
            tf_basic->SetLineWidth(3) ;
            tf_basic->SetLineColor(2) ;
            tf_basic -> Draw("same") ;
            sprintf( func_str, "%8.4f+x*x*(%8.4f)", best_chi2_full_aval, best_chi2_full_bval ) ;
            TF1* tf_full = new TF1( "tf_full", func_str, -1., 1. ) ;
            tf_full->SetLineWidth(3) ;
            tf_full->SetLineColor(4) ;
            tf_full -> Draw("same") ;
