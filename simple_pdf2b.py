...
def simple_pdf2b( theta0, theta_unknown_S, train_train_det_within_sphere, train_train_gen_within_sphere, iterations, scales, radius, neval, verbose=0, compute_radii=True ):
    ...
    neval_train = neval
    neval_true  = neval
    if neval_train > ngen_train :  neval_train = ngen_train
    if neval_true  > ngen_true  :  neval_true = ngen_true
    ...
    if compute_radii :
       ...
       #-- create outside here
       #train_train_det_within_sphere = np.zeros( shape=(ngen_train, neval), dtype=bool )
       #train_train_gen_within_sphere = np.zeros( shape=(ngen_train, neval), dtype=bool )

       delta_det = np.zeros( shape=(ngen_train, ndim) )
       delta_gen = np.zeros( shape=(ngen_train, ndim) )


       for pi in range( neval_train ) :
          if (pi%(neval_train/10) == 0 ) : print('  %12d / %12d' % (pi, neval_train) )
    ...
    train_true_det_within_sphere = np.zeros( shape=(ngen_train, neval_true), dtype=bool )

    for pi in range( neval_true ) :
        if (pi%(neval_true/10) == 0 ) : print('  %12d / %12d' % (pi, neval_true) )
        dist2_det = np.zeros( shape=(ngen_train) )
        for di in range( ndim ) :
            delta_det[:,di] = (theta0_S[:,di] - theta_unknown_S[pi,di] ) / scales[di]
            dist2_det = dist2_det + delta_det[:,di] * delta_det[:,di]
        train_true_det_within_sphere[:,pi] = ( dist2_det < radius )


    for pi in range( ngen_train ) :
        train_true_det_count_within_sphere[pi] = train_true_det_within_sphere[pi].sum()


    true_det_simple_pdf_at_train_points = train_true_det_count_within_sphere / (1.*ngen_true)
    ...
    for i in range(iterations):
        ...
        for pi in range( ngen_train ) :
            ...
            train_det_simple_pdf_at_train_points[pi] = weights_push[0:neval_train][ train_train_det_within_sphere[pi] ].sum()

        train_det_simple_pdf_at_train_points = train_det_simple_pdf_at_train_points / weights_push.sum()

        train_det_simple_pdf_at_train_points = np.clip( train_det_simple_pdf_at_train_points, 1e-11, 1e6)

        step1_output_weights = sample_norm_sf * true_det_simple_pdf_at_train_points / train_det_simple_pdf_at_train_points
...
#       return_dict["train-hist-step1-iter%d" % i] = train_history_step1
        ...
        for pi in range( ngen_train ) :
            ...
            train_gen_simple_pdf_at_train_points_push_weight[pi] = weights_push[0:neval_train][ train_train_gen_within_sphere[pi] ].sum()

        train_gen_simple_pdf_at_train_points_push_weight = train_gen_simple_pdf_at_train_points_push_weight / weights_push.sum()
        ...
        for pi in range( ngen_train ) :
            ...
            train_gen_simple_pdf_at_train_points_pull_weight[pi] = weights_pull[0:neval_train][ train_train_gen_within_sphere[pi] ].sum()

        train_gen_simple_pdf_at_train_points_pull_weight = train_gen_simple_pdf_at_train_points_pull_weight / weights_push.sum()
