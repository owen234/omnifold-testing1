...
#include "corgen.c"
   ...
   void toy_chi2_test1( double a0=0. , double b0=0.4, int nshow=5,  int ngen=10000   ) {
      ...
      TH1F* h_data = new TH1F( "h_data", "data", 3, -1.5, 1.5 ) ;
      ...
      for ( int i=0; i<ngen; i++ ) {
         ...
         for ( int ai=0; ai<nscan; ai++ ) {
            ...
            for ( int bi=0; bi<nscan; bi++ ) {
               ...
               chi2_basic += pow( (x1-(ascan)), 2 ) / covmat(1,1) ;
               chi2_basic += pow( (x2-(ascan+bscan)), 2 ) / covmat(2,2) ;
